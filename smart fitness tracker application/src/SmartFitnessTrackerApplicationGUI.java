					...
					while (true) {
						...
						switch (choice) {
							...
							case 5: // View workout history
								TempConsoleUI.ClearConsole();
								Boolean UserBack_ViewWorkoutHistory = GUI.ViewWorkoutHistory(selectedUser.getWorkouts());
								if (UserBack_ViewWorkoutHistory) {
									break; // Return to main menu
								}
								break;
							...
							case 6: // View progress report
								TempConsoleUI.ClearConsole();
								Boolean UserBack_ViewProgressReport = GUI.ViewProgressReport(selectedUser);
								if (UserBack_ViewProgressReport) {
									break; // Return to main menu
								}
								break;

							case 7: // Log water intake
								TempConsoleUI.ClearConsole();
								Water water = GUI.LogWaterIntake();
								if (water != null) {
							...
							case 10: // View water history
								TempConsoleUI.ClearConsole();
								Boolean ViewWater_IntakeHistory = GUI.ViewWaterIntakeHistory(selectedUser);
								if (ViewWater_IntakeHistory) {
									break; // Return to main menu
								}
								break;
							
							case 11: // View sleep history
								TempConsoleUI.ClearConsole();
								Boolean ViewSleepHistory = GUI.ViewSleepHistory(selectedUser);
								if (ViewSleepHistory) {
									break; // Return to main menu
								}
								break;
							
							case 12: // View weight history
								TempConsoleUI.ClearConsole();
								Boolean ViewWeightHistory = GUI.ViewWeightHistory(selectedUser);
								if (ViewWeightHistory) {
									break; // Return to main menu
								}
								break;
